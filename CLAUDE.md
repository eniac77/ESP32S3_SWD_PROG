...
Kész és fordul: `swd_phy`, `adiv5`, `cortexm_debug`, `flm_runner`, `flm_blobs` (üres tábla), `target_db` (42 STM32 tag), `prog_session` (end-to-end), `storage_lfs` (+ `storage_src` forrás-feloldó), `storage_usb` (USB MSC, Kconfig-gated), `display_oled`, `input_enc`, `ui`, `target_serial`, `target_state`, `net_wifi`, `web_ui`, `ftp_srv`. Tool: `tools/flm_extract.py`.

**USB pendrive forrás (✅ implementálva, Kconfig-gated, default KI):** `CONFIG_USB_MSC_HOST_ENABLE=y` esetén a bedugott FAT32 stick a `/usb` alá mountolódik (`storage_usb`: usb_host_msc + esp_vfs_fat, hot-plug), és ha van stick, a firmware (`/fw`) + config (`/cfg`) lista (OLED + web) a **stickről** jön — a belsőt elrejti; kihúzva visszaáll. A forrásválasztás a `storage_src` rétegen megy (prefix-alapú diszpécs LFS↔USB + a megfelelő lock). A **www** és az **FTP** szándékosan végig a belső LFS-en marad; a forrásfájl a flash/WiFi-pause ELŐTT teljesen PSRAM-ba olvasódik (kihúzás flash közben nem tör). **Pad-ütközés:** OTG host = GPIO19/20 native USB → a másodlagos USB-Serial/JTAG konzolt KI kell kapcsolni (`CONFIG_ESP_CONSOLE_SECONDARY_NONE=y`), konzol UART0-n, flash/monitor külső USB-UART-ról (GPIO43/44); fordítási `#error` véd, ha mégis bent maradna. Default kikapcsolva = nulla viselkedésváltozás (natív USB flash/monitor megmarad). Részletek: a terv 18. szekciója.
...
**Hátralévő:**
- **USB MSC HW-teszt:** a kód mindkét állapotban (host ki/be) zöldre fordul; valódi pendrive-os end-to-end teszt (mount → lista a stickről → flash a stickről → kihúzás-visszaállás) HW-n még hátra (`CONFIG_USB_MSC_HOST_ENABLE=y` + `CONFIG_ESP_CONSOLE_SECONDARY_NONE=y` bilddel, külső USB-UART konzollal).
