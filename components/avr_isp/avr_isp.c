...
esp_err_t avr_isp_flash_file(const char *path, avr_progress_cb cb, void *ctx)
{
    ...
    } else if (ends_with_elf(path)) {
        err = parse_avr_elf((const uint8_t *)raw, raw_len, img, flash_size, &img_len);
        if (err != ESP_OK) { ESP_LOGE(TAG, "AVR ELF parse hiba"); ret = err; goto out; }
        ESP_LOGI(TAG, "AVR ELF betoltve: %u B hasznos", (unsigned)img_len);
    } else {
        /* Nyers .bin 0-tól. */
